...
SENSOR_ID = {
  'RADAR_FRONT': 7, 'RADAR_FRONT_LEFT': 9, 
...
def process_batch(nusc, scene_names, split, start_img_id=0, start_ann_id=0):
    """處理一批場景並返回處理結果"""
    print(f"Processing batch with {len(scene_names)} scenes...")
    ...
    #num_anns = 0
    # 從上一批的ID計數繼續
    num_images = start_img_id
    num_anns = start_ann_id
    #
    # 處理每個場景
    progress_bar = tqdm(scene_names)
    for scene_name in progress_bar:
        # 找到場景的所有樣本
        progress_bar.set_description(f"scene: [{scene_name}]")
        scene_token = None
        for s_token in nusc.scene:
            if nusc.get('scene', s_token['token'])['name'] == scene_name:
                scene_token = s_token['token']
                break
        ...
        while sample_token:
            ...
            # 處理每個相機傳感器的數據
            for sensor_name in USED_SENSOR:
                if sensor_name in sample['data']:
                    ...
                    for radar_channel in RADARS_FOR_CAMERA[sensor_name]:
                        ...
                        # 只保存點的位置和速度信息，不儲存完整點雲
                        for i in range(min(100, radar_pcs.points.shape[1])):
                            point = radar_pcs.points[:, i]
                            radar_pc_list.append({
                                'x': float(point[0]),
                                'y': float(point[1]),
                                'z': float(point[2]),
                                'vx': float(point[3]),
            ...
            if num_images % 100 == 0:
                gc.collect()
            '''
    print(f"Processed {num_images} images and {num_anns} annotations in this batch")
    return ret, num_images, num_anns

def save_batch_results(batch_results, batch_file):
    """保存批次處理結果到文件"""
...
def merge_batch_results(batch_files, output_file):
    """合併所有批次處理結果到最終文件"""
    merged_results = {
        'images': [],
        'annotations': [],
    ...
    for batch_file in batch_files:
        ...
        # 合併類別和屬性信息（只需要第一次）
        if merged_results['categories'] is None:
            merged_results['categories'] = batch_data['categories']
        if merged_results['attributes'] is None:
            merged_results['attributes'] = batch_data['attributes']
        
        # 合併圖像和注釋
        merged_results['images'].extend(batch_data['images'])
        merged_results['annotations'].extend(batch_data['annotations'])
    ...
    # 重新分配ID以確保唯一性
    image_id_map = {}
    for i, img in enumerate(merged_results['images']):
        image_id_map[img['id']] = i + 1
        img['id'] = i + 1
    
    for i, ann in enumerate(merged_results['annotations']):
        ann['id'] = i + 1
    ...
    print(f"Total: {len(merged_results['images'])} images, {len(merged_results['annotations'])} annotations")
...
def main():
    ...
    for split in SPLITS:
        ...
        if split == 'test':
            # 測試分割處理所有場景
            scene_names = [nusc.get('scene', s['token'])['name'] for s in nusc.scene]
        else:
            # 使用預定義的場景列表
        ...
        for i in range(0, len(scene_names), CONVERT_BATCH):
            batch_num = i // CONVERT_BATCH + 1
            batch = scene_names[i:i + CONVERT_BATCH]
            print(f"Processing batch {i//CONVERT_BATCH + 1}/{(len(scene_names) + CONVERT_BATCH - 1)//CONVERT_BATCH}")
            
            # 處理當前批次
            batch_results, global_num_images, global_num_anns = process_batch(
                nusc, batch, split, global_num_images, global_num_anns
            )
            # batch_results = process_batch(nusc, batch, split)
            
            # 保存批次結果
            batch_file = f"{OUT_PATH}/batch_{split}_{i//CONVERT_BATCH}.json"
            save_batch_results(batch_results, batch_file)
        ...
        # 合併所有批次結果
        output_file = f"{OUT_PATH}/{split}_ALL.json"
        merge_batch_results(batch_files, output_file)
...
def only_merge():
    ...
    for split in SPLITS:
        ...
        for i in range(9):
            # 處理當前批次
            batch_file = f"{OUT_PATH}/batch_{split}_{i}.json"
            batch_files.append(batch_file)
